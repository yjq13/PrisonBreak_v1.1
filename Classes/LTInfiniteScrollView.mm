...
- (void)reloadData
{
    ...
    self.showViewCount=[self.dataSource numberOfShowViews];
    ...
    NSInteger begin = -(self.showViewCount / 2.0f)-self.showViewCount%2+1;
    NSInteger end = self.showViewCount / 2.0f;
    _currentIndex = 0;
    ...
    for (UIView *view in self.views) {
        if ([self viewCanBeQueuedForReuse:view]) {
            ...
            if (indexOfViewToReuse < self.currentIndex) {
                indexNeeded = indexOfViewToReuse + self.showViewCount ;
            } else {
                indexNeeded = indexOfViewToReuse - (self.showViewCount );
            }
